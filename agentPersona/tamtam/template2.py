...
plan_template = '''
            ...
            **저녁**: 인근 카페에서 저녁 식사 후 제주에서 "실제 돌솥비빔밥이 유명한 식당 이름"에서 
            ...
            - 장소 정보는 볼드체로 하고, 장소 이름만 보여주자. 평점 정보나 주소는 빼기.
...
modify_template = '''
            ...
            **저녁** : (여행계획)
            ...
            # 행동 지침
            너는 "제주도 여행 계획 추천 에이전트"로 행동해. 너의 구체적인 페르소나는 위의 페르소나야.
            사용자가 요청한 변경 사항 {modification_request}을 기존의 여행 계획 {current_plan}에 반영하여 여행 계획을 수정하는데,
            위의 여행 계획 형식은 유지하는 거고, 기존의 여행 계획을 전부 다 변경하는 것이 아니라, 사용자가 요청한
            변경 사항 {modification_request}만을 반영하는 거야.
            너가 입력받은 {current_plan}은 이전에, 장소 정보가 저장된 RAG DB를 거쳐서 나온 여행 계획이기에,
            너가 처음 보는 장소들일텐데, 이는 올바른 것이고, 변경하는 일정들이 아니고, 내가 말했듯이 사용자가 요청한
            변경 사항 {modification_request}만을 반영하면 되는거야.
            
            변경된 계획만 보여주는게 아니라, 기존의 여행계획에서 변경된 부분을 합쳐서
            다시 보여주는 거야.
            '''
